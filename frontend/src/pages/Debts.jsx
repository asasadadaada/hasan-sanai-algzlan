...
function DebtForm({ initial, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-card border border-border rounded-lg w-full max-w-md p-5" onClick={(e) => e.stopPropagation()} data-testid="debt-form">
                ...
                <td>
                  <div className="flex items-center justify-end gap-1">
                    {d.amount_remaining > 0 && (
                      <button onClick={() => setPaying(d.id)} data-testid={`pay-debt-${d.id}`} title={t("settle")} className="p-1.5 rounded hover:bg-muted"><DollarSign className="w-4 h-4" /></button>
                    )}
                    <button onClick={() => setEditing(d)} data-testid={`edit-debt-${d.id}`} title={t("edit")} className="p-1.5 rounded hover:bg-muted"><Edit className="w-4 h-4" /></button>
                    <button onClick={() => del(d)} data-testid={`delete-debt-${d.id}`} title={t("delete")} className="p-1.5 rounded hover:bg-destructive/10 text-destructive"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </td>
              </tr>
            ))}
