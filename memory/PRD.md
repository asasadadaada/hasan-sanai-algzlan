...
## Session 4 (Apr 2026) — Restore from GitHub
- Cloned repo `asasadadaada/hasan-sanai-algzlan` into `/app`
- Created `/app/backend/.env` with required keys (MONGO_URL, DB_NAME, JWT_SECRET, ADMIN_USERNAME, ADMIN_PASSWORD, DEFAULT_TENANT_NAME, CORS_ORIGINS)
- `pip install -r requirements.txt` & `yarn install` — all clean
- **Bug fixed:** Tailwind 3 cannot parse opacity modifiers on arbitrary HSL values inside `@apply` (`bg-[hsl(... )]/10` syntax). Replaced lines 213-218 of `/app/frontend/src/index.css` with plain CSS using `hsl(... / 0.10)` notation. Frontend now compiles successfully.
- Verified end-to-end: backend `/api/health` 200, login `admin/Admin@2026` returns user+tenant, dashboard renders Arabic RTL UI with KPIs and 30-day chart.
